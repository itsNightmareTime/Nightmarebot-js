...
class Roller {
  ...
  setupPrng(): void {
    let roll = 17;
    const dateNumber = parseInt(strftime('%m%d%y', new Date(Date.now())), 10);
    roll += dateNumber;
    roll += Roller.hashString(this.mapName);
    this.roll = roll;
    for (let i = 0; i < 5; i++) {
      this.rollPrng();
    }
  ...
  static hashString(string: string): number {
    let retval = 0;
    const array: number[] = [];
    ...
    array.forEach((byte, index) => {
      retval += ((index + 1) * byte * ((index + 1) + byte));
    });
    return retval;
  }
